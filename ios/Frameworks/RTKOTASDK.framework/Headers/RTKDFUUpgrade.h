...
/**
 * Tells the delegate to display the result of checking image feature info.
 *
 * @param task The upgrade task reporting this event.
 * @param connection The device connection upon which upgrade is performing.
 * @param success Whether the image feature on SOC is successfully obtained.
 * @param isConsistent Whether the image features between SOC and upgrade images are consistent.
 * @param startHandler The block must be called when delegate finishs its work.
 *
 * @discussion A DFU delegate can use this method to show the feature info consistency before Upgrade send image bytes.
 */
- (void)DFUUpgrade:(RTKDFUUpgrade *)task
        withDevice:(RTKProfileConnection *)connection
showFeatureInfoAvailable:(BOOL)success
       andConsistency:(BOOL)isConsistent
...
/**
 * A boolean value that indicates Whether to check the consistency of the feature info of the SOC and the upgrade file.
 *
