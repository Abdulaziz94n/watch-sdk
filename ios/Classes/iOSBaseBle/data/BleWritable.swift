...
open class BleWritable: BleReadable, BleBuffer {
     ...
     * 写入UInt的低32位。
     */
    func writeUInt32(_ value: UInt, _ order: ByteOrder? = nil) {
        if order ?? mByteOrder == ByteOrder.BIG_ENDIAN {
            writeInt16(Int(value) >> 16, ByteOrder.BIG_ENDIAN)
            writeInt16(Int(value) & 0xffff, ByteOrder.BIG_ENDIAN)
        } else {
            writeInt16(Int(value) & 0xffff, ByteOrder.LITTLE_ENDIAN)
            writeInt16(Int(value) >> 16, ByteOrder.LITTLE_ENDIAN)
        }
    }
     ...
     * 写入UInt，64位。
     */
    func writeUInt(_ value: UInt, _ order: ByteOrder? = nil) {
        if order ?? mByteOrder == ByteOrder.BIG_ENDIAN {
            writeUInt32(value >> 32, ByteOrder.BIG_ENDIAN)
            writeUInt32(value & 0xffffffff, ByteOrder.BIG_ENDIAN)
        } else {
            writeUInt32(value & 0xffffffff, ByteOrder.LITTLE_ENDIAN)
            writeUInt32(value >> 32, ByteOrder.LITTLE_ENDIAN)
        }
    }
     ...
     * @param encoding 编码格式。
     * @param addEllipsis 如果超出最大分配的字节数是否显示省略号, 默认:false
     */
    func writeStringWithLimit(_ text: String?, _ limit: Int, _ encoding: String.Encoding = .utf8, addEllipsis: Bool = false) {
        
        guard let safaText = text, !safaText.isEmpty else {
            return
        }
        
        //skip(8 - mPositions[1]) // 写入的字符序列不允许跨字节，先跳到下个字节的开始位置
        guard let data = safaText.data(using: encoding) else {
            return
        }
        ...
        if data.count <= limit {
            ...
            if addEllipsis {
                ...
                var flag = safaText.count
                while flag > 0 {
                    flag -= 1
                    
                    // 从最后一位开始截取, 每次删除一个字符
                    let subStr = safaText[0..<flag] + "..."
                    if subStr.bytes.count <= limit {
                        
                        writeStringWithFix(subStr, limit)
                        // 退出循环
                        flag = -1
                    }
                }
    ...
    // 2023-08-24之前的代码, 后面增加超过指定长度后最后更改为显示省略号
    //func writeStringWithLimit(_ text: String?, _ limit: Int, _ encoding: String.Encoding = .utf8, addEllipsis: Bool = false) {
    //    if text == nil || text!.isEmpty {
    //        return
    //    }
