...
open class Languages {
    public static let DEFAULT_CODE = 0x01
    public static let INVALID_CODE = 0x1F // R5语言包刷错时，code会变成该值
    public static let DEFAULT_LANGUAGE = "en"
...
//        "invalid": 0x1F  // 1F不能再使用，R5语言包刷错时，code会变成该值
    ...
    // 将语言转换成协议对应的Int值
    public static func languageToCode(
        language: String = String(NSLocale.current.identifier.prefix(2)), default: Int = Languages.DEFAULT_CODE
    ) -> Int {
        
        var code = LANGUAGES[language] ?? `default`
        if language.elementsEqual("zh"){
            //中文,判断是否为繁体
            if NSLocale.current.identifier.range(of: "Hant") != nil ||
                NSLocale.current.identifier.range(of: "TW") != nil ||
    ...
    // 将协议对应的Int值转换成语言
    public static func codeToLanguage(code: Int, default: String = DEFAULT_LANGUAGE) -> String {
