...
open class BlePhoneWorkOut: BleWritable {
    static let ITEM_LENGTH = 32
    
    public var mStep = 0
    public var mDistance = 0 // 1m
    public var mCalories = 0 //1kcal
    public var mDuration  = 0 // 运动持续时长，数据以秒为单位
    public var mSmp = 0  //平均步频
    public var mAltitude  = 0 // 海拔高度，数据以米为单位
    public var mAirPressure = 0 // 气压，数据以 kPa 为单位
    public var mAvgPace = 0 //s/km
    public var mAvgSpeed: Float = 0 //h/km
    public var mModeSport = 0 //运动模式
    public var mUndefined = 0 //占位符
    
    override var mLengthToWrite: Int {
        BlePhoneWorkOut.ITEM_LENGTH
    }
    
    public init(_ step: Int, _ distance: Int, _ calories: Int, _ duration: Int, _ smp: Int, _ altiude: Int, _ airPressure: Int, _ avgPace: Int, _ avgSpeed: Float, _ modeType: Int) {
        super.init()
        mStep = step
        mDistance = distance
        mCalories = calories
    ...
    override func encode() {
        ...
        writeFloat(mAvgSpeed, .LITTLE_ENDIAN)
    ...
    required public init(from decoder: Decoder) throws {
        ...
        mAvgSpeed = try container.decode(Float.self, forKey: .wrokAvgSpeed)
