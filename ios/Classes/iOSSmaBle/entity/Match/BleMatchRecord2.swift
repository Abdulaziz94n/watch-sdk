...
/// 比赛记录2
open class BleMatchRecord2: BleReadable {
    ...
    /// 是否是秒表的数据
    public var mIsStopWatchData: Int = 0
    /// 整个比赛的周期数据
    public var mPeriod: BleMatchPeriod2 = BleMatchPeriod2()
    /// 周期的数据
    public var mPeriodList = [BleMatchPeriod2]()
    /// 事件数据数组 （log data） 如果是秒表数据取前面99*4 byte，没4 byte记录一圈的时间
    public var mLogList = [BleMatchLog2]()
    /// 是秒表数据时用到,毫秒
    public var mStopWatchList = [Int]()
    /// 团队信息
    public var mTeamInfo: BleMatchRecordTeam = BleMatchRecordTeam()
    /// 天气信息
    public var mWeather: BleWeather = BleWeather()
...
public extension BleMatchRecord2 {
