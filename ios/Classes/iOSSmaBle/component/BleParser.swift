...
class BleParser: BleParserDelegate {
    ...
    func onReceive(_ data: Data) -> Data? {
        ...
        bleLog("flag = 004, BleParser onReceive -> total=\(mData.count), mReceived=\(mReceived), data:\(data.count)")
        // 2024-03-16这个是之前旧的判断, 有问题, 修改了了一下
        //if mReceived < mData.count {
        if mData.count >= (mReceived+data.count) {
